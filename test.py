...
def staff_jobs_view_create():
    print("###################################################################################################")
    print("type is coming in this :", current_user.__dict__['email'])
    print("###################################################################################################")

    current_user_obj = Employee.query.filter_by(email=current_user.__dict__['email']).first()
    isRecruiter = Users.query.filter_by(emp_id=current_user_obj.id).first().type
